...
class QualificationEvidenceLifecycle(StrEnum):
    """Append-only lifecycle of one bounded M10 evidence dataset."""
...
class QualificationEvidenceValidity(StrEnum):
    """Current evidence usability, independent of historical graduation results."""
...
class GraduationDisposition(StrEnum):
    """M10's only graduation outcomes; neither enables a trading mode."""

    ELIGIBLE_FOR_DEMO_REVIEW = "ELIGIBLE_FOR_DEMO_REVIEW"
    NOT_ELIGIBLE = "NOT_ELIGIBLE"


class QualificationGateCategory(StrEnum):
    SAMPLE = "SAMPLE"
...
class QualificationComparator(StrEnum):
    EQUAL = "EQUAL"
    LESS_THAN_OR_EQUAL = "LESS_THAN_OR_EQUAL"
    GREATER_THAN_OR_EQUAL = "GREATER_THAN_OR_EQUAL"


class QualificationMetricStatus(StrEnum):
    VALID = "VALID"
    UNCERTAIN = "UNCERTAIN"
    UNAVAILABLE = "UNAVAILABLE"


class QualificationCollectionMode(StrEnum):
...
class QualificationReasonCode(StrEnum):
    ...
    OPERATIONAL_THRESHOLD_FAILED = "OPERATIONAL_THRESHOLD_FAILED"
    SAFETY_BOUNDARY_VIOLATION = "SAFETY_BOUNDARY_VIOLATION"
    COST_THRESHOLD_FAILED = "COST_THRESHOLD_FAILED"
    TRADING_THRESHOLD_FAILED = "TRADING_THRESHOLD_FAILED"
    PARTITION_INELIGIBLE = "PARTITION_INELIGIBLE"


class QualificationIncidentCategory(StrEnum):
    RUNTIME_FAILURE = "RUNTIME_FAILURE"
    PROVIDER_RUNTIME_FAILURE = "PROVIDER_RUNTIME_FAILURE"
    UNEXPLAINED_ABANDONED_CYCLE = "UNEXPLAINED_ABANDONED_CYCLE"
    TRACE_MISMATCH = "TRACE_MISMATCH"
    SCHEMA_VALIDATION_FAILURE = "SCHEMA_VALIDATION_FAILURE"
    SOURCE_REVISION = "SOURCE_REVISION"
    RISK_ENGINE_BYPASS = "RISK_ENGINE_BYPASS"
    CONFIDENCE_RISK_COUPLING = "CONFIDENCE_RISK_COUPLING"
    EXECUTION_OR_ORDER_MUTATION = "EXECUTION_OR_ORDER_MUTATION"
    DEMO_INVOCATION = "DEMO_INVOCATION"
    LIVE_INVOCATION = "LIVE_INVOCATION"
    PROVIDER_FALLBACK = "PROVIDER_FALLBACK"
    STRATEGY_MUTATION = "STRATEGY_MUTATION"
    PROMPT_SELF_MUTATION = "PROMPT_SELF_MUTATION"


class QualificationErrorCategory(StrEnum):
    INVALID_EVIDENCE = "INVALID_EVIDENCE"
    EVIDENCE_NOT_OPEN = "EVIDENCE_NOT_OPEN"
    EVIDENCE_NOT_SEALED = "EVIDENCE_NOT_SEALED"
    EVIDENCE_ALREADY_EVALUATED = "EVIDENCE_ALREADY_EVALUATED"
    DIGEST_MISMATCH = "DIGEST_MISMATCH"
    DEPENDENCY_INCOMPATIBLE = "DEPENDENCY_INCOMPATIBLE"
    ACCEPTANCE_INELIGIBLE = "ACCEPTANCE_INELIGIBLE"
...
class DemoExecutionState(StrEnum):
    """Durable M11 state; only DISPATCHING may precede broker mutation."""
...
class ExecutionControlState(StrEnum):
    """Operator-owned DEMO execution control; disabled is the default."""
...
class ExecutionSnapshotPurpose(StrEnum):
    """Why a later snapshot exists without changing the analysis record."""

    PRE_SEND_REVALIDATION = "PRE_SEND_REVALIDATION"


class DemoOrderExecutionMode(IntEnum):
    """Normalized MT5 symbol execution modes."""
...
class DemoFillingMode(StrEnum):
    """Normalized filling choices used by the narrow DEMO adapter."""
...
class DemoSubmissionDisposition(StrEnum):
    """Normalized result of the single allowed submission attempt."""
...
class DemoReconciliationStatus(StrEnum):
    """Outcome of composite broker-state reconciliation."""
...
class DemoExecutionFailureCategory(StrEnum):
    """Sanitized M11 failure taxonomy."""

    QUALIFICATION_INELIGIBLE = "QUALIFICATION_INELIGIBLE"
    QUALIFICATION_EVIDENCE_INVALID = "QUALIFICATION_EVIDENCE_INVALID"
    FAKE_PROVIDER_EVIDENCE = "FAKE_PROVIDER_EVIDENCE"
    ACCEPTANCE_CHAIN_INVALID = "ACCEPTANCE_CHAIN_INVALID"
    ENVIRONMENT_ACCEPTANCE_EXPIRED = "ENVIRONMENT_ACCEPTANCE_EXPIRED"
    ...
    STALE_EXECUTION_TICK = "STALE_EXECUTION_TICK"
    ...
    BROKER_METADATA_CHANGED = "BROKER_METADATA_CHANGED"
    UNSUPPORTED_EXECUTION_MODE = "UNSUPPORTED_EXECUTION_MODE"
    UNSUPPORTED_FILLING_MODE = "UNSUPPORTED_FILLING_MODE"
    OPEN_POSITION_CONFLICT = "OPEN_POSITION_CONFLICT"
    RISK_REJECTED = "RISK_REJECTED"
    RISK_HALTED = "RISK_HALTED"
    FRESH_VOLUME_MISMATCH = "FRESH_VOLUME_MISMATCH"
    ...
    DISPATCH_RESULT_UNKNOWN = "DISPATCH_RESULT_UNKNOWN"
    PARTIAL_FILL = "PARTIAL_FILL"
    FILL_OUTSIDE_POLICY = "FILL_OUTSIDE_POLICY"
    PROTECTIVE_LEVEL_MISMATCH = "PROTECTIVE_LEVEL_MISMATCH"
...
class ContinuousRuntimeState(StrEnum):
    """Trusted lifecycle state of the bounded M9 coordinator."""
...
class DecisionClaimState(StrEnum):
    """Durable exactly-once lifecycle for one completed decision candle."""
...
class ObservationFailureCategory(StrEnum):
    """Sanitized M9 failure taxonomy."""

    MT5_UNAVAILABLE = "MT5_UNAVAILABLE"
    SYMBOL_TIMESTAMP_INELIGIBLE = "SYMBOL_TIMESTAMP_INELIGIBLE"
    INVALID_COMPLETED_CANDLE = "INVALID_COMPLETED_CANDLE"
    DUPLICATE_CANDLE = "DUPLICATE_CANDLE"
    SOURCE_CANDLE_REVISED = "SOURCE_CANDLE_REVISED"
    SNAPSHOT_INVALID = "SNAPSHOT_INVALID"
    SNAPSHOT_CANDLE_MISMATCH = "SNAPSHOT_CANDLE_MISMATCH"
    SNAPSHOT_ID_CONFLICT = "SNAPSHOT_ID_CONFLICT"
    STALE_SNAPSHOT = "STALE_SNAPSHOT"
    FEATURE_FAILURE = "FEATURE_FAILURE"
    FEATURE_TRACE_MISMATCH = "FEATURE_TRACE_MISMATCH"
    RISK_BASELINE_MISSING = "RISK_BASELINE_MISSING"
    RISK_BASELINE_AMBIGUOUS = "RISK_BASELINE_AMBIGUOUS"
    RISK_CONTEXT_INVALID = "RISK_CONTEXT_INVALID"
    PROVIDER_CONFIGURATION_INELIGIBLE = "PROVIDER_CONFIGURATION_INELIGIBLE"
    PROVIDER_RUNTIME_FAILURE = "PROVIDER_RUNTIME_FAILURE"
    BUDGET_EXHAUSTED = "BUDGET_EXHAUSTED"
    AGENT_FAILURE = "AGENT_FAILURE"
    BACKPRESSURE = "BACKPRESSURE"
    DECISION_TOO_OLD = "DECISION_TOO_OLD"
    PROCESS_RESTART = "PROCESS_RESTART"
    STORAGE_FAILURE = "STORAGE_FAILURE"
    OUTCOME_SOURCE_FAILURE = "OUTCOME_SOURCE_FAILURE"
    UNKNOWN = "UNKNOWN"


class RiskBaselineState(StrEnum):
    """Explicit lifecycle of caller-verified cash-flow-adjusted baselines."""
...
class RiskBaselineProvenance(StrEnum):
    """Accepted non-autonomous sources of M3 baseline facts."""

    OPERATOR_ATTESTED = "OPERATOR_ATTESTED"
    EXTERNAL_LEDGER_RECONCILED = "EXTERNAL_LEDGER_RECONCILED"
    MIGRATED_VERIFIED = "MIGRATED_VERIFIED"


class M9DecisionDisposition(StrEnum):
    """Continuous-cycle outcome before or around the M6 shadow graph."""
...
class OutcomeTrackingState(StrEnum):
    """Separate read-only lifecycle for shadow outcome collection."""
