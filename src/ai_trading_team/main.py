...
from ai_trading_team.config.startup import M5_STARTUP_POLICY, StartupPolicyError
from ai_trading_team.utils.logging import configure_logging, get_logger


def main() -> int:
    """Validate M5 configuration and exit without provider or trading activity."""
    try:
    ...
    logger.info(
        "m5_startup_validated",
        extra={"app_mode": settings.app_mode.value, "milestone": "M5"},
    )
